...
def ctx(workspace, protected_paths=None):
    """A ToolContext with no session.

    These tests exercise the edit mechanics themselves; read-before-write is a
    session-scoped guarantee and is covered against the real loop in
    test_read_before_write.py.
    """
    return ToolContext(workspace=workspace, protected_paths=list(protected_paths or []))
...
def test_replace_generates_diff_and_applies(tmp_path):
    ...
    proposal = build_edit_proposal(ctx(tmp_path, []), {"path": "a.py", "old_text": "return 1", "new_text": "return 2"})
...
def test_replace_requires_unique_match(tmp_path):
    ...
    with pytest.raises(EditError, match="not unique"):
        build_edit_proposal(ctx(tmp_path, []), {"path": "a.py", "old_text": "x = 1", "new_text": "x = 2"})


def test_replace_missing_old_text(tmp_path):
    (tmp_path / "a.py").write_text("x = 1\n", encoding="utf-8")
    with pytest.raises(EditError, match="not found"):
        build_edit_proposal(ctx(tmp_path, []), {"path": "a.py", "old_text": "not there", "new_text": "y"})


def test_create_new_file(tmp_path):
    proposal = build_edit_proposal(ctx(tmp_path, []), {"path": "new/b.py", "new_text": "print(1)\n"})
...
def test_create_existing_file_rejected(tmp_path):
    ...
    with pytest.raises(EditError, match="old_text"):
        build_edit_proposal(ctx(tmp_path, []), {"path": "a.py", "new_text": "y"})


def test_delete_file(tmp_path):
    target = tmp_path / "a.py"
    target.write_text("x", encoding="utf-8")
    proposal = build_edit_proposal(ctx(tmp_path, []), {"path": "a.py", "delete": True})
...
def test_stale_detection(tmp_path):
    ...
    proposal = build_edit_proposal(ctx(tmp_path, []), {"path": "a.py", "old_text": "x = 1", "new_text": "x = 2"})
...
def test_protected_path_rejected(tmp_path):
    with pytest.raises(EditError):
        build_edit_proposal(ctx(tmp_path, [".env"]), {"path": ".env", "new_text": "SECRET=1"})
...
def test_runtime_secret_value_cannot_be_written(tmp_path, monkeypatch):
    ...
    with pytest.raises(EditError, match="sensitive Runtime environment"):
        build_edit_proposal(ctx(tmp_path, []), {"path": "config.py", "new_text": 'KEY = "provider-secret-value"\n'})
...
def test_non_utf8_file_rejected_without_corruption(tmp_path):
    ...
    with pytest.raises(EditError, match="UTF-8"):
        build_edit_proposal(ctx(tmp_path, []), {"path": "a.bin", "old_text": "y = 2", "new_text": "y = 3"})

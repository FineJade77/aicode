// Package sessionscmd lists persisted sessions and applies the retention policy.
package sessionscmd
...
func ParseArgs(args []string) (Options, error) {
	...
	for ; index < len(args); index++ {
		...
		if !known {
			return Options{}, fmt.Errorf("unknown flag for `aicode sessions`: %s\n\n%s", arg, usage)
		}
		if !hasInline {
			if index+1 >= len(args) {
	...
	if !opts.Prune && (opts.MaxSessions != nil || opts.MaxAgeDays != nil) {
		return Options{}, fmt.Errorf("--max-sessions and --max-age-days only apply to `aicode sessions prune`\n\n%s", usage)
	}
	if opts.Prune && (opts.Limit != nil || opts.Offset != nil) {
		return Options{}, fmt.Errorf("--limit and --offset only apply to `aicode sessions`\n\n%s", usage)
	}
	return opts, nil
}

...
| T-047 repo map / 符号索引 | `[ ]` | 触发条件（定位失败占比显著）未满足。**决定它的曲线首轮作废，待重跑**——见 T-016 的 `live_scale_curve` 记录。 |
...
缺陷已改为**交叉引用错误**（handler 过滤另一个真实 handler 的名字），文件内每个 token 在别处都出现，单次搜索无法择出。新增 `test_no_module_carries_a_token_its_peers_lack` 挡住这类破绽，并排除模块名派生的标识符以免误报。四点已重新验证起始为红且可解，**曲线待重跑**。

**教训：出题人留下的捷径会伪装成被测对象的能力。** 一条平坦的曲线和一条因为答案卡而平坦的曲线，在报告里长得一模一样。
...
唯一非通过率的信号是 `live_scale` 的 4× 检索成本，而把它变成可判断数字的那条曲线待重跑。**下一步是重跑 `live_scale_curve`，不是启动 T-047。**
...
`live_scale_curve` 就是为把这个决定变成数字而建的，但**首轮曲线因出题破绽作废**（缺陷含唯一 token，一次 grep 即命中），缺陷已改为交叉引用错误并加测试挡住，待重跑。

启动判据：重跑后的增长指数（log 效率 / log 模块数）。接近 0 → 规模基本免费，本任务降级；明显上翘 → 检索就是成本，转第一优先。**在拿到这个数字之前不启动。**

...
完成记录（2026-08-01）：交付物为 [docs/review/2026-08-01-live-eval-report.md](docs/review/2026-08-01-live-eval-report.md)，数字已写入 README 顶部。

**结果：28 任务 × 3 重复 = 84 次运行全部通过，pass@1 = pass@3 = 1.000，总成本 $0.65，p95 42.0s，失败归因为空集**（模型 `deepseek-v4-pro`）。
...
**因此 T-023 / T-024 / T-026 / T-047 / T-048 的触发条件仍未满足**，它们依赖的是失败归因的分布，而分布是空的。据此启动仍属拍脑袋，只是多了一张好看的成绩单作掩护。缺的是难度不是机制——harness 本身已被证明有效，本轮由它逼出 5 个真 bug（其中 2 个在产品而非评测里：policy 路径探测 ENAMETOOLONG 崩溃终结整个 turn、模型别名导致计价静默丢失）。

首轮（`tool: reject`）的 27/28 已作废：那一轮 Agent 一次命令都跑不了，代码是盲写的、验证是 grader 做的。本轮 157 次 bash 调用 0 次被拒、155 次真实执行，确认 approval 修复生效。

后续（2026-08-01 已落地一半）：**`live_hard` 一档已建成**——8 个任务，难点是因与果的距离：失败的测试指向的模块不是该改的那个；难度来自间接层级而不是代码量，强模型读代码很快。类别按"难在哪儿"命名（`localization` 2 / `cross_module` 2 / `algorithmic` 2 / `reproduce_first` 1 / `underspecified` 1），因此分类别通过率**直接回答被 gate 的那个问题**。与 `live` 分开而非替换：后者是零成本回归地板与已知参照点，合并会让每次运行为两者付钱。8 个全部由 `reference_solutions_hard.py` 的已知可行解验证可解——**难必须是难而不是无解**，这一档尤其要紧，因为失败本身就是产物，分不清"模型不行"和"题出错了"就没有信号。`make eval-live-hard` 可跑。
...
**设计假设被证伪**："因与果的距离"不构成难度。跨三个模块的间接层级对能快速读完整个仓库的模型几乎零成本——中位工作量与易档几乎相同（1.4 次编辑 / 2.4 条命令 / 7.4 次模型调用，易档是 1.4 / 2.1 / 6.1）。我按"人读代码要跳几次"估难度，模型不是那样读的。

**T-023 / T-024 / T-026 / T-047 / T-048 触发条件仍未满足**：能力维度的归因分布依然是空的。归因从空集变成 `{safety_violation: 1}`，那回答的是"越了界"而不是"在哪里失败"。
...
该轮另暴露两个工程问题（均已修）：(1) **eval 把开发机 PATH 泄进被测对象**——Agent 执行 `python -m pytest` 拿到 Python 2.7，自行诊断切到 `python3` 但花掉 7 次模型调用里的 2 次；harness 用 digest 钉住了 fixture/任务/prompt/profile，却把 PATH 原样交出去，同一套任务集换台机器行为就变。已在 `source_versions` 记录 `agent_toolchain`，**刻意不进 baseline**（每台机器不同，钉住会让别的机器全挂）。(2) `glob` 对 `**test*` 只报 pathlib 的规则不给改法，已附修正建议。

**首轮曲线作废（2026-08-02）**：测得完全平坦（工具调用增长指数 −0.01），但那量的是出题破绽而非 Agent 能力——缺陷模块含有 `weight` 这个仅它与测试文件才有的 token，模型 grep 一次即命中，与规模无关的 O(1) 查找（trace 中 query 的 sha256 与 `weight` 完全吻合）。缺陷已改为**交叉引用错误**（handler 过滤另一个真实 handler 的名字），文件内每个 token 在别处都出现，单次搜索无法择出。新增 `test_no_module_carries_a_token_its_peers_lack` 挡住这类破绽。四点已重新验证起始为红且可解，**曲线需重跑**。教训：出题人留下的捷径会伪装成被测对象的能力。

**已落地 `live_scale_curve`（2026-08-01）**：把规模档从一个点做成一条曲线——同一个缺陷分别埋进 10 / 30 / 100 / 300 个模块的仓库，缺陷本身、任务描述、预算、模型全部固定，只有模块数在变（由测试钉住，否则曲线量的是漂移不是规模）。缺陷刻意不在首尾：靠习惯就能找到的位置不算检索。`scripts/eval_curve.py` 从报告读出效率–规模表与增长指数。**这条曲线是回答 T-047 值不值得做的直接证据**：接近平坦说明规模基本免费、repo map 收益有限；明显上翘说明检索就是成本。四个点均验证起始为红且可解。

`live_scale` 首轮结果（2026-08-01）：**2/2 通过，规模也不是能力瓶颈**。但首次出现非平凡信号,不在通过率而在**检索成本**:工具调用从易档 6.8 涨到规模档 27.0（4×），模型调用只从 6.1 涨到 9.5（1.6×）——模型没多想，是在多找。详见报告第 6 节。**T-047 的书面触发条件（定位失败占比显著）仍未满足**；现有的是成本信号而非失败信号，是否据此启动需要显式决定，不能用"差不多满足"代替。

**已按此结论落地 `live_scale`（2026-08-01）**：2 个任务、每个 30+ 个近乎相同的模块（35 handler 中 1 个违反返回契约；30 配置段中 2 个拼错必填键）。grep 症状命中几十处同样合理的位置，填充代码长度刻意一致（测试钉住同组模块最长与最短差 ≤ 2 行），**无法靠形状认出问题模块**。两个任务均验证起始为红且可解。`make eval-live-scale` / `make eval-live-scale-openai-compatible` 可跑。**若这一档能区分而 `live_hard` 不能，那正是 T-047 在等的证据。**

原分析（换轴方向）：规模超出通读舒适区（几十文件、接近上下文窗口，正是 T-047 要回答的）、需要多处协同编辑、仓库内没有 oracle 的真模糊需求、长时程多步。另可先用更弱模型跑同一套 `live_hard`（`OC_MODEL=` 覆盖）——若弱模型出现分布性失败，说明任务集有区分度只是对该模型太易，那是 T-026 的直接输入且无需新出题。

...
本文是**任务粒度的执行记录**：每个任务的背景、范围、验收标准和完成记录。能力层面的状态总览见 [ROADMAP.md](ROADMAP.md)，本文不重复。

完成记录写的是**为什么这么做**，包括踩过的坑和被证伪的假设。这些内容的价值在于它们无法从代码反推——代码只显示最终形态，不显示当初为什么排除了另外三种。
...
全部 48 项中已关闭 43 项。剩余 5 项：

| 任务 | 状态 | 卡在哪里 |
| --- | --- | --- |
| T-014 Anthropic prompt caching | `[!]` | 代码与测试已完成。缺的是验收要求的**量化对照数据**，而本机没有 Anthropic key、当前 provider 不认 `cache_control`。属环境阻塞，不是实现问题。 |
| T-023 检索式项目记忆 | `[ ]` | 触发条件未满足：评测未显示跨会话重复解释造成可观测的 token 浪费。 |
| T-026 模型横向对比与上下文消融 | `[ ]` | 指标已实现，缺有区分度的任务集。弱模型对照已做且结果与强模型完全一致。 |
| T-047 repo map / 符号索引 | `[ ]` | 触发条件未满足，且**曲线已跑出并否掉了这根轴**（2026-08-04）：检索成本次线性增长且在 100 模块处走平。降级，见该条。 |
| T-048 subagent | `[ ]` | 触发条件未满足：评测未显示主上下文被探索过程显著污染。 |

T-024（plan / todo 工具）已由 T-033 交付，条目保留但标为作废，见该条。
...
7. **由证据触发的任务不得靠直觉启动**：T-023 / T-026 / T-047 / T-048 各自写明了触发条件，条件未满足时启动仍属拍脑袋，一张好看的成绩单不构成理由。

## 阅读须知

**里程碑编号有一处跳空**：M7 在 2026-07-29 的优先级调整中被重命名为 M11（见 M4 节的调整说明）。保留跳空而不重编号——现网记录和历史 commit 引用的是原编号。

**任务来源文档不在版本库内**。T-001~T-012 来自 `LOCAL_AGENT_ROADMAP.md`，T-013~T-032 与 T-033~T-048 来自 `docs/review/` 与 `docs/plans/` 下带日期的评审与计划。这些文件都位于 `docs/`，而 `docs/` 在 `.gitignore` 里，因此**只在本地检出中存在**；本文中指向它们的链接对克隆者是死链，保留是为了本地可追溯。

**包结构说明（2026-07-30）**：Runtime 已完成一次包重组——`app/core/`、`app/adapters/`、`app/config/`、`app/contracts/` 不再存在，port 与 domain 类型并入 `app/agent/`，adapter 实现按领域就近放置，composition root 为 `app/bootstrap.py`。本台账中指向**现存文件**的路径已按新结构更新；T-001~T-012 的完成记录描述的是当时的产物，保留原路径并在需要处注明新位置。带日期的快照文档整体保留当时的路径，不做改写。当前结构见 [ARCHITECTURE.md](ARCHITECTURE.md) 第 3.4 节与第 21 节。
...
**T-014（prompt caching）与 T-016（真实模型评测）已从第一梯队降级至此**（2026-07-29 目标切换为生产可用）；两者都不阻塞生产可用。T-023 / T-024 / T-026 三项**在 T-016 产出真实评测数字之前不启动**，届时按数据决定取舍。此纪律的原始出处为 `LOCAL_AGENT_ROADMAP.md`（该文件已不在版本库内）：索引、subagent、自动规划框架只由评测结果触发。

**本节的后续演进（2026-08-02）**：T-016 已交付并跑出四档结果，但结论是"通过率没有区分度"，因此上述三项的触发条件**依然未满足**——数字有了，分布是空的。T-024 另因与 T-033 重复而作废。详见各条。

### `[!]` T-014 Anthropic prompt caching

对应：评审 B3（2026-07-29 由第一梯队降级：属成本增效而非正确性问题，不阻塞生产可用）

依赖：无。**阻塞在环境而非实现**：代码与测试已完成，缺的是验收要求的量化对照数据。
...
完成记录（2026-08-01）：交付物为 [docs/review/2026-08-01-live-eval-report.md](docs/review/2026-08-01-live-eval-report.md)，数字已写入 README。

本任务的产出不是代码而是**结论**，因此下面按四档任务集的演进顺序记录，而不是按提交顺序。一句话总结：**四档全部功能性满分，通过率已经不携带信息；唯一在动的是检索成本，而量它的那条曲线首轮量到的是出题破绽。**

#### 第一档 `live`（28 任务）：满分，但没有区分度

首轮（DeepSeek，`REPETITIONS=1`）27/28 通过，p95 35.4s。**这一轮的价值不在数字而在它暴露的三个 bug**（见下文"由评测逼出的 bug"）。其中 approval 策略那个是出题 bug，修复后重跑。

正式轮（`deepseek-v4-pro`，28 × 3 = 84 次运行）：**全部通过，pass@1 = pass@3 = 1.000，总成本 $0.65，p95 42.0s，失败归因为空集**。

**这个满分必须连着它的局限一起记**：全过意味着任务集对该模型**没有区分度**，因此它并未回答 roadmap 真正要问的问题——Agent 在哪里失败。证据：`retry_fix` 类 12 次运行中 9 次一改即过、**11/12 次在跑任何测试之前就改完了**，为"必须读失败再改"设计的链路基本没被走到；单次运行平均只有 6.1 次模型调用、1.4 次编辑。

缺的是**难度**而不是机制——harness 本身已被证明有效，本轮由它逼出 5 个真 bug，其中 2 个在产品里。

#### 第二档 `live_hard`（8 任务）：设计假设被证伪

假设是"**因与果的距离**构成难度"——失败的测试指向的模块不是该改的那个，难度来自间接层级而不是代码量（强模型读代码很快）。类别按"难在哪儿"命名（`localization` 2 / `cross_module` 2 / `algorithmic` 2 / `reproduce_first` 1 / `underspecified` 1），因此分类别通过率**直接回答被 gate 的那个问题**。

与 `live` 分开而非替换：后者是零成本回归地板与已知参照点，合并会让每次运行为两者付钱。8 个全部由 `reference_solutions_hard.py` 的已知可行解验证可解——**难必须是难而不是无解**，这一档尤其要紧，因为失败本身就是产物，分不清"模型不行"和"题出错了"就没有信号。

首轮结果（2026-08-01，`deepseek-chat`，报告：[docs/review/2026-08-01-live-hard-eval-report.md](docs/review/2026-08-01-live-hard-eval-report.md)）：**难度提升失败。8/8 功能性解决**，pass@1 = 0.875，唯一判负是策略违规而非能力不足（改对了实现，同时改了禁止修改的测试文件）。

**假设被证伪**：跨三个模块的间接层级对能快速读完整个仓库的模型几乎零成本——中位工作量与易档几乎相同（1.4 次编辑 / 2.4 条命令 / 7.4 次模型调用，易档是 1.4 / 2.1 / 6.1）。我按"人读代码要跳几次"估难度，模型不是那样读的。

第二轮换弱模型（`deepseek-v4-flash`）结果**完全一致**：8/8 功能性解决、pass@1 = 0.875、同一处策略违规。这排除了"任务集有区分度只是对强模型太易"——**换弱模型没让分布散开，任务集本身没有区分度**。同一任务在两个模型上以同样方式越界，说明那是稳定倾向而非随机。

这一轮的直接产物之一是报告新增 `functional_pass_at_1` 与 `policy_only_failures`：只记录判负结果，会让"解出来但越界"和"根本没解出来"长得一样，而在难度校准上这两者结论正好相反。

#### 第三档 `live_scale`（2 任务）：换轴——让"读完所有东西"不再便宜

`live_hard` 证伪了间接层级构成难度，因此换轴。候选方向有四个：规模超出通读舒适区、需要多处协同编辑、仓库内没有 oracle 的真模糊需求、长时程多步。先做规模，因为它正是 T-047 要回答的那根轴。

2 个任务、每个 30+ 个近乎相同的模块（35 handler 中 1 个违反返回契约；30 配置段中 2 个拼错必填键）。grep 症状命中几十处同样合理的位置，填充代码长度刻意一致（测试钉住同组模块最长与最短差 ≤ 2 行），**无法靠形状认出问题模块**。两个任务均验证起始为红且可解。

首轮结果（2026-08-01）：**2/2 通过，规模也不是能力瓶颈**。但首次出现非平凡信号，不在通过率而在**检索成本**：工具调用从易档 6.8 涨到规模档 27.0（4×），模型调用只从 6.1 涨到 9.5（1.6×）——**模型没多想，是在多找**。

**T-047 的书面触发条件（定位失败占比显著）仍未满足**；现有的是成本信号而非失败信号，是否据此启动需要显式决定，不能用"差不多满足"代替。

#### 第四档 `live_scale_curve`（4 点）：把那个决定变成数字

通过率既然不再携带信息，就直接量会动的那个量。同一个缺陷分别埋进 10 / 30 / 100 / 300 个模块的仓库，缺陷本身、任务描述、预算、模型全部固定，只有模块数在变（由测试钉住，否则曲线量的是漂移不是规模）。缺陷刻意不在首尾：靠习惯就能找到的位置不算检索。`scripts/eval_curve.py` 从报告读出效率–规模表与增长指数（log 效率 / log 模块数）。

**这条曲线是回答 T-047 值不值得做的直接证据**：接近平坦说明规模基本免费、repo map 收益有限；明显上翘说明检索就是成本。

**首轮曲线作废（2026-08-02）**：测得完全平坦（工具调用增长指数 −0.01），但那量的是**出题破绽而非 Agent 能力**——缺陷模块含有 `weight` 这个仅它与测试文件才有的 token，模型 grep 一次即命中，是与规模无关的 O(1) 查找（trace 中 query 的 sha256 与 `weight` 完全吻合）。

缺陷已改为**交叉引用错误**（handler 过滤另一个真实 handler 的名字），文件内每个 token 在别处都出现，单次搜索无法择出。新增 `test_no_module_carries_a_token_its_peers_lack` 挡住这类破绽，并排除模块名派生的标识符以免误报。

**第二轮曲线也作废（2026-08-04）：同一类破绽换了个藏法。** 重跑得到 4/4 通过、指数 −0.08 / 0.00 / 0.07，仍然平坦；但读 trace 发现 300 模块下模型只读了三个文件就改完，对 handlers 目录一次搜索都没做。原因是**失败的测试第一条断言直接写着缺陷模块名**：

```python
assert summarise(records_for("export_batch"))["processed"] == 1   # 缺陷就在 export_batch.py
```

读测试即拿到答案，定位成本按构造为零，与规模无关——曲线必然平坦。**唯一 token 被堵死后，答案key 从模块内容转移到了测试文件里。**

缺陷已改为**对奇偶切片的聚合失衡**：缺陷 handler 位于偶数下标、错引一个奇数下标的名字，于是偶数批少算一个、奇数批多算一个。失败信息是纯标量 `assert 149 == 150` / `assert 151 == 150`，不点任何模块名，四档形状完全一致。缺陷位置同时重排到各档 registry 中部，`reference_solutions_curve.py` 同步重生成并验证四档起始为红且可解。

新增两条守卫：`test_the_failing_test_names_no_handler`（测试文件不得出现任何 handler 名）与 `test_the_failure_message_does_not_grow_with_the_repository`（四档失败输出长度差 < 200 字符）。后者防的是**反方向**的陷阱——把 `len(kinds)` 写进断言会让 pytest 把整个切片展开进 transcript，300 档凭空多出 150 个名字，会伪造出一条上升的 input_tokens 曲线。两个方向的污染都能让曲线"看起来有结论"。

**正式曲线（2026-08-04，`deepseek-chat` 经 DeepSeek，4 点 × 3 次 = 12 次运行，$0.34）**：

| 模块数 | 工具调用 | 模型调用 | input tokens | 秒 |
| ---: | ---: | ---: | ---: | ---: |
| 10 | 15.0 | 9.7 | 42,619 | 22.1 |
| 30 | 16.7 | 14.0 | 83,073 | 33.4 |
| 100 | 22.0 | 20.0 | 128,386 | 47.6 |
| 300 | 21.3 | 19.7 | 137,245 | 45.8 |

增长指数：工具调用 0.12、模型调用 0.22、input tokens 0.35。**12/12 全部通过，归因分布仍是空集。**

**结论：检索成本确实随规模增长，但是次线性的，而且在 100 模块处就走平。** 规模涨 30 倍，input tokens 只涨 3.2 倍、工具调用只涨 1.4 倍；100 与 300 两点在 n=3 下无法区分。

平台不是压缩造成的（12 次运行 0 次 compaction），而是**工具输出 2000 字符上限**：300 档有 11 次输出撞上限，10 档只有 2 次。规模增长被这层写入截断转成了更多次调用，而不是更多 token——三级上下文管理的第一级在这里直接把检索成本的斜率压平了。

**这条曲线的局限必须连着结论一起记**：300 个约 15 行的模块合计仍只有约 137k input tokens 分摊在 20 次调用上，仓库整体没有超出上下文窗口。它量的是"大量近乎相同的模块"，不是"仓库远大于上下文"。每点 3 次重复，单模型。

**教训：出题人留下的捷径会伪装成被测对象的能力，而且堵一处它会换一处。** 一条平坦的曲线和一条因为答案卡而平坦的曲线，在报告里长得一模一样——两轮都是先读 trace 才发现的，报告本身两次都显示 PASS。

#### 由评测逼出的 bug

harness 有效性的实证。5 个真 bug 中 2 个在产品里：

- **policy 路径探测崩溃（产品 bug）**。`policy.py` 把 shell 命令的每个 token 都当候选路径试探，`Path.exists()` 吞 ENOENT 但不吞 ENAMETOOLONG。模型内联一段 579 字符的 `python3 -c` 脚本时，异常从 policy 抛出、**越过 tool 错误处理、终结整个 turn**（`session.final` 都没发出）——模型写一段长脚本的代价是丢掉整个会话，而不是拿回一次失败的工具调用。已改为不抛异常的探测；`path_like` token 仍照常按解析后的文本 gate，由 `cat ../aaa…` 仍然 deny 的测试钉住。回归测试先复现生产报错再验证修复。
...
- **eval 把开发机 PATH 泄进被测对象（harness bug）**。Agent 执行 `python -m pytest` 拿到 Python 2.7，自行诊断切到 `python3` 但花掉 7 次模型调用里的 2 次；harness 用 digest 钉住了 fixture/任务/prompt/profile，却把 PATH 原样交出去，同一套任务集换台机器行为就变。已在 `source_versions` 记录 `agent_toolchain`，**刻意不进 baseline**（每台机器不同，钉住会让别的机器全挂）。
- **`glob` 对 `**test*` 只报 pathlib 的规则不给改法**（产品体验），已附修正建议。
- **provider 中断被归因成 Agent 失败（harness bug，2026-08-04）**。重跑曲线时 DeepSeek 连续返回 HTTP 503「Service is too busy」，3/4 个点在几秒内中止。retry 本身是对的（503 在 `RETRYABLE_STATUS` 内，3 次尝试带等抖动退避），但退避窗口总共约 1.5 秒，扛不住 provider 级繁忙——这是"快速失败交给用户"的设计取舍，不是缺陷。缺陷在**报告**：中止的 run 被记为 `agent_error`，与"Agent 自己出错"在唯一一张用来读触发条件的表里完全同形。已新增 `provider_unavailable` 归因档（从 trace 确定性判定：`ProviderError` + 可重试状态码或传输失败，状态集从 `app.models.provider.RETRYABLE_STATUS` 导入以免两处对"瞬时"的定义漂移），优先级在 `budget_exhausted` 之下、`agent_error` 之上；报告顶部加横幅说明所有比率都把中止 run 算了进去；`scripts/eval_curve.py` 遇到中止 run 直接拒绝出图。**最后一条是关键**：中止的 run 把工作量拉向 0，也就是拉向"规模免费"那个方向，那一轮实际打印出了 −0.83「flat」的假曲线。

#### 出题 bug（harness 自身的修正）

- **approval 策略让评测测错了东西**。原先 24 个作业任务是 `tool: reject`，模型**一次命令都跑不了**，代码是盲写的、验证是 grader 做的。于是 `retry_fix` 不再测"读失败再改"，`verification_failure` 永远不可能触发。已改为作业任务 `tool: accept`、安全任务保持 `reject`（拒绝本身是被测行为），并由测试钉住这个划分。policy 仍直接拒绝危险可执行文件与 protected path，accept 放宽的是可批准范围而非可执行范围。**首轮 `tool: reject` 的 27/28 因此作废**；正式轮 157 次 bash 调用 0 次被拒、155 次真实执行，确认修复生效。
- **长上下文种子未按窗口比例化**，已修。
- **clamp mutation 是"单点 off-by-one"**，改为"整体移除文档行为"——前者可能被一份恰好不覆盖该点的合理测试漏掉。
- `reference_solutions.py` 新增**第二份写法不同的解**交叉校验 mutation——只校验一份参考解，分不出 mutation 写得对还是恰好对上了那份解挑的输入。

#### 触发条件评估（当前结论）

**T-023 / T-026 / T-047 / T-048 的触发条件仍未满足。** 它们依赖失败归因的分布，而四档跑下来分布始终是空的（`live_hard` 从空集变成 `{safety_violation: 1}`，但那回答的是"越了界"而不是"在哪里失败"）。据此启动仍属拍脑袋，只是多了一张好看的成绩单作掩护。

**曲线已跑出（2026-08-04）**：检索成本随规模次线性增长（input tokens 指数 0.35，工具调用 0.12），并在 100 模块处走平。**T-047 不启动**——书面触发条件（定位失败占比显著）未满足，且现在连成本信号也表明规模不是瓶颈：规模涨 30 倍，检索只贵 3.2 倍且已走平。详见上文第四档记录。
...
- **新增 `evals/reference_solutions.py`**（范围外但必要）：每个 live 任务一份已知可行解，测试套件校验其可解且每条 mutation 都被抓到。无解任务报出来的是出题人的 bug 而不是模型的失败，两者必须能区分——已验证 24/24 可解、10/10 mutation 被抓。为控制这一步的耗时，参考解校验关掉了 pytest 插件自动加载（1.9s → 0.5s/次，fixture 只用核心 pytest 特性）；grader 本身刻意不这么做，它跑的必须和 Agent 跑的一致。
...
**当前证据状态（2026-08-02）**：未满足。四档 live 都是单会话任务，跨会话的重复解释成本在这套任务集里根本不会出现——**触发条件所依赖的现象超出了现有评测的观测范围**，因此"未满足"在这里的含义是"没测过"，不是"测了没有"。要真正判断，需要的是使用日志中的 token 分布，而不是再出一档题。

### `[x]` T-024 plan / todo 工具（作废：由 T-033 交付）

对应：评审 B5 | 原触发条件：T-016 的失败归因集中在"跑偏 / 漏做子任务"

原范围：轻量 todo 工具，主要价值是用户可见性而非模型记忆；Go renderer 渲染为进度清单。

**本条作废（2026-08-02）**：T-033「显式计划状态」已经交付了完全相同的东西——`update_plan` 工具、`plan.updated` 事件、Go renderer 的进度清单渲染、随 session 持久化。两条任务来自两轮不同的评审（B5 与 L1），在当时没有被识别为同一件事。

保留条目而不删除，是因为它记录了一个判断上的教训：T-024 被挂在"等评测触发"上，而 T-033 被判定为"不需要评测数据即可判断该做"——**同一个能力，两轮评审给出了相反的排期结论**。区别在于论证方式：T-024 说的是"可能有用"，T-033 说的是"模型意图在 Runtime 内没有任何外部表示"，后者指的是一个已知缺失的机制，因此不需要数据。
...
**部分已做（2026-08-02）**：`live_hard` 已跑过强弱两个模型（`deepseek-chat` 与 `deepseek-v4-flash`），结果**完全一致**——8/8 功能性解决、pass@1 = 0.875、同一处策略违规。

这个"完全一致"本身就是本任务的第一个结论，只是方向相反：横向对比要有意义，前提是任务集能把模型区分开。**在有区分度的任务集出现之前，跑第三个模型只会得到第三条重合的线。** 因此本任务的实际前置依赖已从 T-016 变为"某一档评测首次出现分布性失败"。
...
**当前证据状态（2026-08-02）**：书面触发条件未满足——四档 live 的归因分布是空的。唯一相关信号是 `live_scale` 的检索成本 4×（工具调用 6.8 → 27.0，模型调用只 6.1 → 9.5），但那是成本信号不是失败信号。

**曲线结论（2026-08-04，前两轮均因出题破绽作废，详见 T-016 第四档记录）**：增长指数为 input tokens 0.35、工具调用 0.12、模型调用 0.22，且在 100 模块处走平（100 与 300 两点在 n=3 下无法区分）。规模涨 30 倍，检索只贵 3.2 倍。

**判定：不启动，本任务降级。** 两个条件都指向同一边——归因分布仍是空集（12/12 通过），成本曲线也次线性且已走平。repo map 要压缩的那项成本，在这根轴上既不构成失败也不快速增长。

**但这个"不"有明确边界**：曲线顶点的 300 个模块合计仍只有约 137k input tokens，仓库整体没有超出上下文窗口；走平还有一部分来自工具输出 2000 字符上限把规模转成了更多次调用。**因此被否掉的是"模块数量多"这根轴，不是"仓库大到装不下"那根轴。** 若日后要重开本任务，前置是先造出后一种规模——单个文件或整体体量远超上下文窗口——而不是把模块数从 300 加到 1000。

### `[x]` T-048 subagent（2026-08-05 交付；**触发条件未满足，由用户决定启动**）

触发条件：T-016 显示主上下文被探索过程显著污染。**测量结果是未满足**，记录在此以免下次有人再问时重跑。

**42 次真实运行的测量**（五档 live 的全部 trace）：

| 指标 | 值 |
| --- | ---: |
| 探索类工具输出（read_file/search/glob/list_files/related_files） | 156,435 字符 |
| 行动类工具输出（bash/edit_file/…） | 120,193 字符 |
| 探索占比 | 57%（单次最高 94%） |
| **发生压缩的运行** | **0 / 42** |
| 因上下文导致的失败 | 0 |

探索按**份额**确实占多数，但**一次代价都没产生**：零压缩、零溢出、零归因。最大单次探索总量 11,581 字符 ≈ 3,300 token，对着 65k 窗口。按 T-047 已经立过的判例（`live_scale` 的 4× 检索成本被判为"成本信号不是失败信号"，不足以启动），57% 的份额同样不够。

**但这个"零"有局限**：现有 fixture 最大只有 300 个十几行的模块，仓库整体从未超出上下文窗口。**在一个从不压缩的环境里测"压缩痛不痛"，得到零是必然的，不是信息。** 这与 T-047 记录的局限是同一条。

**启动理由是用户的产品决定，不是证据。** 规则 7 防的是执行者拍脑袋，不是防所有人做决定；把它记成"证据充分"才是违规。

交付内容与边界（`app/agent/subagent.py`）：

- **只读工具白名单**（`read_file` / `search` / `glob` / `list_files` / `related_files`）。这不是"以后再放开"的限制：一个用户看不见的运行里弹出审批、去改一个不出现在主 transcript 里的文件，那是名义上的审批。只读工具本就不需要审批，问题因此不存在。**白名单执行两遍**——过滤 schema，以及在执行前再拒一次未列出的名字，因为模型可以喊出没给过它的工具名。
- **预算从父轮的剩余里切，不是新发一份**（25%）。否则"派生子 agent"就是绕过单轮花费上限的办法。子 agent 的消耗直接加到父 `TurnLedger` 上。
- **深度为 1**：子 agent 拿不到 `explore`。对一个价值尚未证实的功能，不值得引入树状预算核算的失败模式。
- **policy / ExecutionService / trace 全部复用父 runtime 的**，符合本条目原本就写明的前置要求。
- `subagent.started` / `subagent.finished` 按四处规则登记，CLI 显示模型调用数、工具调用数与花费——用户看不见的工作仍然要可核算。

**「改完自动 review」的提示词规则：能力有效，规则不触发（2026-08-05 实测）。**

用户提出的用法是"提示词要求修改完用 subagent review 代码"。能力侧已打通并直接验证过——把 `review_diff` 加入子 agent 白名单后，审阅子 agent 能拿到真实的确定性发现（在一个含 `os.system` 的 diff 上报出 medium 级问题）。

**上述两轮 0/8 的测量作废（2026-08-05）：`explore` 从未被注册进 registry。** ToolSpec 声明了、loop 也接管了执行，但 `build_default_registry()` 里没有它，于是 `schemas_for_mode` 不含它——**模型的工具表里根本没有这个工具**，而 `spec_for` 返回 `None` 又让 policy 判定 `unknown tool` 直接 deny。两个方向都堵死。模型不是不听话，是没有这个选项。

**单元测试全程是绿的**，因为它们直接驱动 `run_subagent` / `execute_subagent`——**测了零件，没测通路**。已补 `test_every_declared_tool_is_reachable_by_the_model`（声明集必须等于注册集）与 `test_the_subagent_tools_are_offered_and_permitted`（既在工具表里，又不被 policy 拒）挡住复发。

修复后重测，这才是第一次有效测量：

| 措辞 | 工具可见 | 运行 | 触发 |
| --- | --- | ---: | ---: |
| 独立一条规则，带条件与豁免 | **否（bug）** | 4 | 0 —— 无效 |
| 折进"改完跑测试"那条 | **否（bug）** | 4 | 0 —— 无效 |
| 折进"改完跑测试"那条 | 是 | 4 | **0** |

`live_coordinated` 每题都改 4 个文件，正是规则针对的场景。同一句话里的"跑测试"部分每次都照做（bash 2–3 次/题），"call explore" 部分被忽略。结论方向未变，但**证据量从 8 次降到 4 次**，且只在一个模型上测过。

按规则 3（行为变更必须说明评测覆盖，无覆盖不得宣称质量提升）：**不能声称"现在会自动自审"**。规则保留（约 50 token，可能在更强模型上生效），但结论按实测记：0/4，且只测过一个模型。

**这次的教训值得单列**：我把一个自己制造的 bug 读成了关于模型行为的发现，还写进了台账。区别在于我当时只验证了"子 agent 能跑"（直接调函数），没验证"模型能调到它"。**功能是否可达，必须沿模型实际走的那条路验证。**

**可靠的触发方式是显式调用**——用户直接说"review 一下改动"，或把审阅流程写成一个 skill 由用户按需调用。自动化这件事需要的不是更好的措辞，而是一个不依赖模型自觉的机制（例如在 verification 通过后由 loop 直接发起），那是另一项工作。

**`delegate`（2026-08-05）：把活交出去，而不只是查资料。** 用户澄清目标是"多任务时让 subagent 去干"，review 只是例子。这需要 worker 能写，而当初设成只读的理由是子 agent 直接调 `runtime.tools.run`、**绕过了 policy 与审批**。

正确的解法不是放宽白名单，而是让子 agent 的每次工具调用走父运行的 `execute_gated`：同一 policy 判定、同一审批弹窗、同一事件流、同一审计。**只有消息历史独立**——那是这个功能的全部意义，也应当是唯一独立的东西。实测一次 delegated `edit_file`：`approval.requested` → `edit.applied`，文件真改，`applied_edits` 回报父轮并计入验证。

**明确不做并行。** 并行写入会同时引入三种竞态——同一文件的冲突写、并发审批、`session.read_files` 上的 read-before-write 记录——而它们都落在共享的 session 状态上。只读的 `explore` 要扇出是安全的，写入不是。

**下一步不是扩展它，是验证它。** 真要判断有没有用，需要一档能制造真实上下文压力的评测（整体体量超出窗口、必须多轮压缩），否则我们同样没有能证明它有效的证据——那就是又一个"没有区分度的满分"。

沿用同一条纪律（原始出处为 `LOCAL_AGENT_ROADMAP.md`，该文件已不在版本库内）：索引、subagent、自动规划框架只由评测结果触发。

任何 subagent 必须复用预算、Policy、ExecutionBackend、approval 与 trace——否则它就是一条绕过全部闸门的旁路，而那正是这套架构在防的东西。

## 后续候选

以下方向不进入当前关键路径，需由 eval 或用户需求触发：

- 受控扩展系统。
- Session export / import（**fork 已由 T-046 交付**，branch 语义即 fork）。
- 大仓库下的索引增量更新策略（前置：T-047 先被触发）。
- 全屏 TUI、Web UI、云端执行。

已从本清单移出的条目：`symbol/import/test mapping` 与 `subagent` 分别归入 T-047 与 T-048，不再在此重复列出——同一件事出现在两处会产生两套优先级。

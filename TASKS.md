...
下一步换轴而非加间接：规模超出通读舒适区（几十文件、接近上下文窗口，正是 T-047 要回答的）、需要多处协同编辑、仓库内没有 oracle 的真模糊需求、长时程多步。另可先用更弱模型跑同一套 `live_hard`（`OC_MODEL=` 覆盖）——若弱模型出现分布性失败，说明任务集有区分度只是对该模型太易，那是 T-026 的直接输入且无需新出题。

...
**仍缺真实运行数字**：跑一次 `make eval-live-hard` 才能知道 pass@1 是否落进 0.4–0.8。若仍接近满分，需要再加一档（更大仓库、接近上下文窗口的代码量）；若接近 0，说明出题过头，按失败归因分布回调。T-023 / T-024 / T-026 / T-047 / T-048 的触发条件在拿到这一档的归因分布之前仍未满足。

...
但**提示词规则在 deepseek-chat 上一次都没触发**：

| 措辞 | 运行 | 触发 |
| --- | ---: | ---: |
| 独立一条规则，带条件与豁免 | 4 | **0** |
| 折进"改完跑测试"那条（该条每次都执行） | 4 | **0** |

`live_coordinated` 每题都改 4 个文件，正是规则针对的场景。同一句话里的"跑测试"部分每次都照做（bash 2–3 次/题），"call explore" 部分被忽略——**差别不在位置，在于模型对这个新工具没有先验**。

按规则 3（行为变更必须说明评测覆盖，无覆盖不得宣称质量提升）：**不能声称"现在会自动自审"**。规则保留（约 50 token，可能在更强模型上生效），但结论按实测记：0/8，且只测过一个模型。

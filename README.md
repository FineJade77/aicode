...
2026-08-01，28 个任务 × 3 次重复 = 84 次运行，模型 `deepseek-v4-pro`（完整报告：[docs/review/2026-08-01-live-eval-report.md](docs/review/2026-08-01-live-eval-report.md)）：
...
**这个满分要连着它的局限一起读**：全过意味着这套任务集对该模型**没有区分度**，因此它还回答不了"Agent 在哪里失败"。证据是 `retry_fix` 类 12 次运行中 9 次一改即过、11 次在跑任何测试之前就改完了——为"必须读失败再改"设计的那条链路基本没被走到。harness 本身是有效的（本轮由它逼出的 5 个真 bug 见报告第 5 节），**缺的是难度而不是机制**。因此依赖失败归因触发的 T-023 / T-024 / T-026 / T-047 / T-048 仍不启动。
...
在 `.aicode/config.json` 声明服务器，它们的工具会自动进入 Agent 的工具集：
...
工具暴露为 `mcp__files__<tool>`，与内置工具走**同一条** policy gate 和审批链路。
...
- 单个服务器起不来、协议违规或调用超时都不影响其它服务器和主流程；起不来会明确报告，而不是让它的工具悄悄消失。

...
| 会话上下文预算 | `[x]` | session 快照带 `context` 块（模型、窗口、已用、是否将压缩），供 TUI/WebUI 直接渲染。 |
...
| `live_coordinated`（4 任务） | `[x]` | 一处改动必须同时落在多处、每处改法不同。**4/4 通过，第四根轴同样不构成瓶颈。** |
| `live_ambiguous`（3 任务） | `[x]` | 需求有两种同样站得住的读法时，会不会先问。**3/3 判负，2/3 根本没问——四轮以来第一份非空信号。** |
| `live_longhorizon`（3 任务） | `[x]` | 十几步顺序工作会不会漏、会不会提前收工。**3/3 通过；序列被压平了。** |
...
- 出题方向从"规模"转向剩下三根还没测过的轴。**"多处协同编辑"已测（`live_coordinated`，2026-08-04）：4/4 通过，pass@1 = 1.000，归因分布仍是空集。** 每处改法不同、少改一处即失败、失败测试不点名文件——这三条都由守卫钉住，所以这次的满分不是出题破绽换来的。**第四根轴同样不构成瓶颈。**
- **"无 oracle 的模糊需求"已测（`live_ambiguous`，2026-08-04）：3/3 判负，其中 2/3 在没有提问的情况下直接选了一个分支实现。这是四轮出题以来第一份非空的失败信号。** 前四档全部满分，而这一档的问题不在"能不能写对代码"——三份实现都能跑通它们各自的读法——而在**替用户做了决定**。新增 `unasked_assumption` 归因档，因为把它记成 `edit_failure` 会让下一个读报告的人去看 diff，而 diff 是对的。

  这一档的设计与前四档不同：**起始为绿**（失败的测试会指向该改哪里，等于泄露意图），分支断言写在 task JSON 里而不是 fixture 里（否则模型读测试即知答案，跳过提问还照样得分），由 `test_the_workspace_does_not_contain_the_answer` 钉住——这条守卫在跑之前就抓到了 `ambig_dedupe` 的一处泄露。

- **"长时程多步"已测（`live_longhorizon`，2026-08-04）：3/3 通过。结论是这根轴上的"长"并不存在——序列会被压平。** `chain_accumulate` 的 14 个依次累加的余额只花了 **6 次模型调用、2 条命令**：模型读完 delta 一次算完全部写回，而不是跑一次改一步。`chain_validate` 的 16 处修正同样是 6 次模型调用。唯一没被压平的是 `chain_discover`（39 次工具调用），因为 12 个文件的写入本身无法合并——那是机械成本，不是时程成本。

  **这一档出题时就承认了压平是允许的**：确定性的链条对能读懂规则并自行模拟的 agent 永远可压缩，要堵住它得靠人为的不透明。所以量的是**完成度**——十几步里会不会漏掉几步就宣布done——而不是"能否阻止压缩"。预算刻意放宽到每步 8 次模型调用以上，否则跑不完会被记成放弃，那是另一个问题。

**五根轴全部测完。** 通过率在四根轴上满分，唯一非空的失败信号来自 `live_ambiguous`——问题不在能力而在**替用户做决定**。四轮出题的经验是同一条：先读 trace 确认模型走的是设计中的那条路径，再看报告数字——两轮平坦曲线在报告里都显示 PASS，破绽只有 trace 能看出来。

...
- **trust 门控**：服务器只在 `trusted` workspace 启动。清单来自被检查仓库自己的 `.aicode/config.json`，因此一个未受信任的检出否则就能指定 daemon 要启动的进程；这与 `.aicode` hooks 在 untrusted 下完全不执行是同一条规则，不另立答案。
...
**生命周期**：服务器由 `McpToolProvider` 按 workspace 缓存，在每个 run 开始时惰性启动（`DefaultToolRuntime.prepare`），跨 run 复用，并在 `ApplicationRuntime.aclose` 时停止。按 workspace 而不是全局建键，是因为两个 workspace 可以声明同名服务器却提供不同工具，调用必须落到自己 workspace 启动的那个进程上。
...
**第一档：写入时截断。** 工具输出在落库前就按上限截断，明确标注截断量（bash 8k、默认 6k）。

`read_file` 不走这条通用限额，它按**行**分页（默认 200 行、上限 500 行）。但**行不是尺寸单位**——压缩后的 bundle、单行 JSON、base64 都是几行几兆。所以它自己有两个额外的轴：单行上限 `MAX_READ_LINE_CHARS = 2000`、单次总量上限 `MAX_READ_TOTAL_CHARS = 64000`，两者都会在返回头里明说（"N long line(s) were clipped"、"stopped at N characters"），恢复方式沿用它本来就会给的 `continue with offset=`。实测：3 行 × 40 万字符的文件，加限之前返回 120 万字符（约 34 万 token），之后 6 千字符。

被裁剪必须说出口：一行被悄悄截短读起来和完整的一行一样，模型据此构造的 `old_text` 会指向文件里并不存在的内容。
...
**引用要能被跟随。** 占位符带上调用本身的标识，而不只是工具名：

```
[folded to save context: read_file app/loop.py:40-239 (9000 characters). Re-read that range if it still matters]
```

参数本来就在历史里（发起调用的 assistant 消息带着 `arguments`），所以标出身份不花额外代价，却把这一档从"有损裁剪"变成模型能照着执行的引用。

**强制压缩时 `keep` 可以到 0。** 常规路径最少保留 1 组，但强制路径必须能丢掉**最新**那一组：一条超大的工具结果既折不掉（任何"保留最近 N 组"的规则都护着它）、也压不掉（`FORCED_KEEP_RECENT_GROUPS = 1` 同样护着它），于是 prompt 超窗 → 重试发送**完全相同**的内容 → 再次超窗 → 整轮失败。允许折叠最新组是安全的，恰恰因为占位符点名了它替换掉的那次调用——模型拿到的是一条可重复的指令，不是一个洞。
